...
| A **question** in the workspace hangs, or its trace arrives all at once at the end | **Fixed, 2026-09-21 (D104) — if you see this again, check the headers first.** `/api/projects/[id]/ask` had no heartbeat, so a single model call that stayed silent past Railway's five-minute no-data cut had its connection closed under it; `effort: "deep"` can do that. It now streams through `src/lib/ask/stream.ts`, which spreads `SSE_HEADERS` and sends a keepalive every 20s. Its `Cache-Control` always did carry `no-transform` — an earlier note here said otherwise and was wrong. Confirm with `curl -sSI` on the response: `cache-control` must contain `no-transform`, and `x-accel-buffering` must be `no`. |
| `vestracode.site` works but `vestracode.site/sign-in` is a 404 | **Open as of 2026-09-21.** The apex host serves `/` and 404s every other path, and intermittently 301s to `http://www.vestracode.site` — note the **`http`**, an insecure hop. `https://www.vestracode.site` is completely healthy: `/`, `/sign-in`, `/app` (307 to sign-in) and `/api/health` (`{"status":"ok","database":"up"}`) all answer correctly. So a stranger who types the bare domain and clicks 로그인 gets a 404. The apex is **not pointed at Railway at all** — it answers from `server: awselb/2.0` in **us-west-2**, which is a registrar's domain-forwarding service, and forwarding only knows how to redirect the root. `www` answers from `server: railway-hikari`. This is a **domain configuration** problem, not application code: add the apex as a second custom domain on the same Railway service (so it gets a Railway certificate and reaches the app on every path) and drop the registrar's forwarding, or keep forwarding and make it a full-path redirect to **`https://`**. Either way `BETTER_AUTH_URL` and both OAuth callback URLs must name whichever host is canonical, or sign-in breaks on the other one. |
