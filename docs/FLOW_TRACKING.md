...
1. ~~**`Focus` is `{ id, lit: Set<string> }` and `linkStrength` is the min of its two ends.**~~ **Done, 2026-09-21 — this is no longer work the flow has to do.** `render/scene.ts` carries `Trail` / `TrailStep` / `stepFor`: an ordered set of *links*, matched on direction as well as on ends, so the `A→B→C` case with a shortcut `A→C` cannot be drawn ambiguously — `linkStrength` returns `DIM` for a link that is not a hop. Hop numbers are exempt from both LOD budgets, verified at 375/900/1400px on the 1,442-item fixture.

   **Two rules the flow must carry over.** A `FlowHop` whose `joint` is not null becomes a `TrailStep` with **`connected: false`**: a joint is `route → file → symbol` collapsed, no edge in the graph has those ends, and drawing a line there would invent a connection (D79). And the gap `drawThread` cuts is **no longer always at the midpoint** — both `drawThread` and `drawRelation` now take the fraction along the line where the word sits, because the label field may slide a word aside to dodge another and a gap left behind at the middle would read as a broken line.
